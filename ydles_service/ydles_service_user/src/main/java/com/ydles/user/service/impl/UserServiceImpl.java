package com.ydles.user.service.impl;

import com.alibaba.fastjson.JSON;
import com.ydles.order.pojo.Task;
import com.ydles.user.dao.PointLogMapper;
import com.ydles.user.dao.UserMapper;
...
public class UserServiceImpl implements UserService {
    ...
    public int addUserPoints(Task task) {
        ...
        //3.将任务存入到redis中
        stringRedisTemplate.boundValueOps(task.getId()+"").set("exist",30, TimeUnit.SECONDS);

        //4.修改用户积分
        int result = userMapper.addUserPoints(username,point);
        if (result<=0){
            return 0;
        }
        ...
        //6.删除redis中的任务信息
        stringRedisTemplate.delete(task.getId()+"");
